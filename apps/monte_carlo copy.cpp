...
{   
    ...
    for(int i = 0; i < config.totalParticles; i++) {
        particles.push_back(Walker(config.originX, config.originY, 0, config.windowConfig.width, 0, config.windowConfig.height));
    }
...
void MonteCarlo::run() {
    ...
    if (timer.isDue()) {
        ...
        for (int i = 0; i < config.totalParticles; i++) {
            ...
            particles[i].setPos(particles[i].getPosX() + dX, particles[i].getPosY() + dY);
            ...
            window.setPixel(particles[i].getPosX(), particles[i].getPosY(), red, green, blue, 0xff);
        }
